...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!user_id || !category_id) {
      return new Response(
        JSON.stringify({ error: "user_id and category_id are required" }),
        {
          status: 400,
    ...
    console.log(
      `Checking spending limits for user ${user_id} in category ${category_id}...`
    );
    ...
    for (const limit of limits) {
      try {
        ...
        if (expensesError) {
          console.error(
            "Error fetching expenses for limit:",
            limit.id,
            expensesError
          );
          continue;
        ...
        console.log(
          `Limit ${
            limit.name
          }: $${totalSpent}/$${limitAmount} (${percentage.toFixed(1)}%)`
        );
        ...
        if (percentage >= 100) {
          ...
          try {
            ...
            // Extract base email for Gmail + aliases (for testing with Resend)
            // This converts kocovskioliver1234+anything@gmail.com -> kocovskioliver1234@gmail.com
            const baseEmail = user.email.includes("+")
              ? user.email.replace(/\+[^@]+@/, "@")
              : user.email;

            const emailResponse = await resend.emails.send({
              from: "Expense Tracker <onboarding@resend.dev>",
              to: [baseEmail], // Use base email for Resend test mode
              ...
              html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #ffffff;">
                  ...
                  <p style="font-size: 16px; color: #374151; margin-bottom: 20px;">
                    Your spending limit "<strong>${
                      limit.name
                    }</strong>" for the <strong>${categoryName}</strong> category has been exceeded.
                  </p>
                  ...
                  <div style="background-color: #fee2e2; border: 1px solid #fca5a5; border-radius: 8px; padding: 20px; margin: 20px 0;">
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                      <tr>
                        ...
                        <td style="padding: 8px 0; color: #374151; text-align: right;">$${limitAmount.toFixed(
                          2
                        )}</td>
                      </tr>
                      <tr>
                        <td style="padding: 8px 0; color: #374151; font-weight: 600;">Amount Spent:</td>
                        <td style="padding: 8px 0; color: #dc2626; text-align: right; font-weight: 600;">$${totalSpent.toFixed(
                          2
                        )}</td>
                      </tr>
                      <tr>
                        <td style="padding: 8px 0; color: #374151; font-weight: 600;">Exceeded by:</td>
                        <td style="padding: 8px 0; color: #dc2626; text-align: right; font-weight: 600;">${percentage.toFixed(
                          1
                        )}%</td>
                      </tr>
                      <tr>
                        ...
                        <td style="padding: 8px 0; color: #374151; text-align: right;">${
                          limit.period_type.charAt(0).toUpperCase() +
                          limit.period_type.slice(1)
                        }</td>
                      </tr>
                    </table>
                  </div>
                  ...
                  <p style="font-size: 16px; color: #374151; margin-bottom: 20px;">
                    Consider reviewing your spending in this category to stay within your budget for the remainder of this ${
                      limit.period_type
                    } period.
                  </p>
