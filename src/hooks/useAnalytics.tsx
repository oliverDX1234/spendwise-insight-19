...
export function useExpensesAnalytics() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Daily expenses
      const dailyExpenses = eachDayOfInterval({ start: monthStart, end: monthEnd }).map(day => {
        const dayString = format(day, 'yyyy-MM-dd');
        const dayExpenses = expenses?.filter(exp => exp.expense_date === dayString) || [];
      ...
      const recurringTotal = expenses?.filter(exp => exp.is_recurring).reduce((sum, exp) => sum + Number(exp.amount), 0) || 0;
      const oneTimeTotal = expenses?.filter(exp => !exp.is_recurring).reduce((sum, exp) => sum + Number(exp.amount), 0) || 0;
      ...
      const expensesByCategory = Array.from(categoryMap.entries()).map(([name, data]) => ({
        name,
        amount: data.amount,
        fill: data.color,
      }));
      ...
      return {
        ...
        totalAmount: expenses?.reduce((sum, exp) => sum + Number(exp.amount), 0) || 0,
      };
    },
    enabled: !!user?.id,
  });
...
export function useCategoriesAnalytics() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Get all categories with expense count and total amount
      ...
      const categoryStats = categories?.map(category => {
        ...
        return {
          ...
          totalAmount: allExpenses.reduce((sum, exp) => sum + Number(exp.amount), 0),
          monthlyAmount: monthlyExpenses.reduce((sum, exp) => sum + Number(exp.amount), 0),
        };
      }) || [];
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const productStats = products?.map(product => {
        ...
        const totalQuantity = allExpenseProducts.reduce((sum, ep) => sum + ep.quantity, 0);
        const monthlyQuantity = monthlyExpenseProducts.reduce((sum, ep) => sum + ep.quantity, 0);
        const totalSpent = allExpenseProducts.reduce((sum, ep) => sum + (ep.quantity * Number(ep.price_per_unit)), 0);
        const monthlySpent = monthlyExpenseProducts.reduce((sum, ep) => sum + (ep.quantity * Number(ep.price_per_unit)), 0);
